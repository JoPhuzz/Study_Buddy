...
BRIEF = (
    ...
    "KEEP THE LOCATORS. Where the notes carry a position — a video timestamp like "
    "[12:40], a page number, a section heading, a capture number — carry it through and "
    "attach it to the fact it belongs to. These are the only way back to the source, and "
    "a claim nobody can go and check is worth a fraction of one they can. They also cost "
    "almost nothing to keep. Never strip them while smoothing fragments into prose: "
    "stitching is about removing repetition, not about removing the map.\n\n"
    "USE THE ROOM YOU HAVE. You are not being scored on brevity, and a short brief is a "
    "lossy one. If the notes run to tens of thousands of words, the brief should be long "
    "in proportion — err toward keeping a detail you are unsure about rather than cutting "
    "it, because nothing here can be recovered later. The captures are thrown away; this "
    "document is what remains.\n\n"
    "KEEP THEIR OWN WORDS SEPARATE. Some captures are marked as written by the person "
    "themselves — their notes, opinions, workflows, things they already knew. Record "
    "those in full, but keep them attributed: 'they note that…', 'their workflow is…'. "
    "Never let something they wrote turn into something a document said, or the "
    "reverse. If their note contradicts a captured source, that is a disagreement to "
    "record, not an error to resolve.\n\n"
